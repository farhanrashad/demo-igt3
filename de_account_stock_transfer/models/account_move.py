...
class AccountMove(models.Model):
    ...
    is_owner = fields.Boolean(related='journal_id.is_owner')
    owner_id = fields.Many2one('res.partner', string="Owner", compute='_get_owner', readonly=False, store=True,)

    picking_type_id = fields.Many2one('stock.picking.type', 'Picking Type',                                     compute='_get_picking_type', )
    #picking_type_code = fields.Selection(related='picking_type_id.picking_type_code')
    ...
    def _get_picking_type(self):
        if self.journal_id.type == 'purchase':
            self.picking_type_id = self.journal_id.receipt_picking_type_id.id
        elif self.journal_id.type == 'sale':
            self.picking_type_id = self.journal_id.delivery_picking_type_id.id
        else:
    ...
    def _get_owner(self):
        for line in self:
            if line.is_owner:
                line.owner_id = line.partner_id.id
            else:
                line.owner_id = False
