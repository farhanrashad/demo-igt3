...
class PurchaseAdvancePaymentInv(models.TransientModel):
    ...
    def _default_product_id(self):
        product_id = self.env['ir.config_parameter'].sudo().get_param('de_purchase_bill.default_purchase_product_id')
    ...
    amount = fields.Float('Down Payment Amount', digits='Account', help="The percentage of amount to be Billed in advance, taxes excluded.")
    currency_id = fields.Many2one('res.currency', string='Currency', default=_default_currency_id)
    fixed_amount = fields.Monetary('Down Payment Amount (Fixed)', help="The fixed amount to be Billed in advance, taxes excluded.")
    ...
    def create_invoices(self):
        ...
        if self.advance_payment_method == 'delivered':
            moves = purchase_orders.action_create_invoice()
        else:
            # Create deposit product if necessary
            if not self.product_id:
                vals = self._prepare_deposit_product()
                self.product_id = self.env['product.product'].create(vals)
                self.env['ir.config_parameter'].sudo().set_param('de_purchase_bill.default_purchase_product_id', self.product_id.id)
            ...
            for order in purchase_orders:
                ...
                if self.product_id.purchase_method != 'purchase':
                    raise UserError(_('The product used to Bill a down payment should have an Control policy set to "Ordered quantities". Please update your deposit product to be able to create a deposit Bill.'))
                if self.product_id.type != 'service':
                    raise UserError(_("The product used to Bill a down payment should be of type 'Service'. Please use another product or update this product."))
