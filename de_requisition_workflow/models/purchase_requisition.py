...
class PurchaseRequisition(models.Model):
    ...
    stage_id = fields.Many2one('purchase.requisition.stage', string='Stage', store=True, readonly=False, ondelete='restrict', tracking=True, index=True, default=_get_default_stage_id, domain="[('requisition_type_ids', '=', type_id)]", copy=False)
        ...
        for requisition in self:
            if requisition.type_id:
                if requisition.type_id not in requisition.stage_id.requisition_type_ids:
                    requisition.stage_id = requisition.stage_find(requisition.type_id.id, [])
            else:
    ...
    def stage_find(self, section_id, domain=[], order='sequence'):
        ...
        if section_ids:
            ...
            for section_id in section_ids:
                search_domain.append(('type_id', '=', section_id))
        search_domain += list(domain)
        # perform search, return the first found
        return self.env['purchase.requisition.type'].search(search_domain, order=order, limit=1).id
