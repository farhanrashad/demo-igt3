...
class EmployeeOvertimeRule(models.Model):
    ...
    def onchange_employees(self):
        if self.employee_ids:
            existing_overtime_rule = self.env['hr.employee.overtime.rule'].search([('overtime_type_id','=', self.overtime_type_id.id)])
            if existing_overtime_rule:
                for ovt in existing_overtime_rule:
                    if ovt.overtime_type_id.type == self.overtime_type_id.type:
