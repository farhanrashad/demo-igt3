...
class AccountCustomEntry(models.Model):
    ...
    category_id = fields.Many2one('approval.category', related='custom_entry_type_id.category_id', string="Category", required=False)
    ...
    def _compute_approvers_count(self):
        ...
        for subscription in self:
            subscription.approvers_count = can_read and Approvers.search_count([('custom_entry_id', '=', subscription.id)]) or 0
    ...
    def action_submit(self):
        ...
        for subscription in self:
            request_list.append({
                ...
                'custom_entry_id': subscription.id,
