...
class ReportAgedPartnerBalance(models.AbstractModel):
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move, period_length, currency_id):
        ...
        currency_obj = self.env['res.currency'].search([('active', '=', True), ('id', '=', currency_id)])
        ...
        if default_currency.id != currency_id:
            for i in range(len(total)):
                total[i] = round(total[i] * currency_obj.rate, 2)
            for mvline in res:
                mvline['direction'] = round(mvline['direction'] * currency_obj.rate, 2)
                mvline['0'] = round(mvline['0'] * currency_obj.rate, 2)
                mvline['1'] = round(mvline['1'] * currency_obj.rate, 2)
                mvline['2'] = round(mvline['2'] * currency_obj.rate, 2)
                mvline['3'] = round(mvline['3'] * currency_obj.rate, 2)
                mvline['4'] = round(mvline['4'] * currency_obj.rate, 2)
                mvline['total'] = round(mvline['total'] * currency_obj.rate, 2)
    ...
    def _get_report_values(self, docids, data=None):
        ...
        movelines, total, dummy = self._get_partner_move_lines(account_type, date_from, target_move,
                                                               data['form']['period_length'], currency_id)
        return {
            'doc_ids': self.ids,
            'doc_model': model,
            'data': data['form'],
