...
class ReportAgedPartnerBalanceExcel(models.Model):
    ...
    def generate_xlsx_report(self, workbook, data, obj):
        ...
        movelines, total, dummy = report_obj._get_partner_move_lines(account_type, date_from, target_move,
                                                                     data['form']['period_length'], data['currency_id'])
        ...
        sheet.write('I6', "Currency", format3)
        row = 6
        ...
        if movelines:
            ...
            sheet.write(row, col + 8, str(currency_obj.name) + "(" + str(data['currency_symbol']) + ")", format5)
        ...
        for partner in movelines:
            ...
            sheet.write(row, col + 8, str(currency_obj.name) + "(" + str(data['currency_symbol']) + ")", format7)
            row += 1
