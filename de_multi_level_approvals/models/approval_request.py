...
class ApprovalRequestInherit(models.Model):
    ...
    def action_refuse(self, approver=None):
        ...
        self.write({'date_confirmed': fields.Datetime.now(), 'request_status':'refused'})
        self.sudo()._get_user_approval_activities(user=self.env.user).action_feedback()    
        approver_count = 0
        for approverid in self.approver_ids:
            if approverid.status == 'new':
...
class ApprovalApproverInherit(models.Model):
    ...
    stage_id = fields.Many2one('approval.stage', compute='_compute_approval_stage', )

    @api.depends('user_id')
    def _compute_approval_stage(self):
        for approver in self:
            approval_category_line = self.env['approval.category.line'].search([('approval_category_id','=',approver.request_id.category_id.id),('user_id','=',approver.user_id.id)],limit=1)
            approver.stage_id = approval_category_line.stage_id.id

        #for approval in self:
            #stage_id = self.env['approval.category.line'].search([('approval_category_id','=',approval.request_id.category_id.id),],limit=1)
